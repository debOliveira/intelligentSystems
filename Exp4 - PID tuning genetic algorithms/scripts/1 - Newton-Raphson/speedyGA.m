...
len=16;                    % The length of the genomes  
...
grid;
plot(k,f(k),'*','LineWidth',1.5,...
    'color',lineStyles(2,:));
legend("f(x)", "f(k)");
title({'{\bf Analog to Newton-Raphson method using genetic algorithms}'; ...
        ['{\it\fontsize{10} Zero found at k} = ',num2str(k)]},...
         'FontWeight','Normal');
ax = gca;
...
%%
function fitness=fitting(pop,f)
    val=binArray2dec(pop); 
    val=abs(f(val)); 
    fitness=(1./val)';
end

function y=binArray2dec(x)
    global len
    y = (-1).^x(:,1).*hex2dec(binaryVectorToHex(x(:,2:len)))/(2^(len-3));
end
